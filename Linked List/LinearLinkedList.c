...
// remember that in simple linear linked list, for inserting or deleting a node, we need to access the PREVIOUS node for performing the operation
...
int insert_afterpos(struct node **ptrnode1ptr, int pos, float new_value){
    if(pos==0){ insert_beg(ptrnode1ptr, new_value); }
    else{
        struct node *p = *ptrnode1ptr;
...
int delete_beg(struct node **ptrnode1ptr, float *del_value){
    if(*ptrnode1ptr==NULL){ return -1; }
        ...
        *del_value = (*ptrnode1ptr)->data;
        struct node *newnode = (*ptrnode1ptr)->next;
        free(*ptrnode1ptr);
        *ptrnode1ptr=newnode;
        return 1;
    }
...
int delete_end(struct node **ptrnode1ptr, float *del_value){
    struct node *p = *ptrnode1ptr;
    if(*ptrnode1ptr!=NULL && (*ptrnode1ptr)->next!=NULL){ // i.e. in linked list 0 or 1 node only
