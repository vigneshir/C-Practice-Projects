...
void BinDiv_Restoring( char *Q, char *M);
...
void Dec_To_2sCompBin(int num, char *twos_comp_bin){
    if (num>0){
        ...
        printf("\nRestoring Division algorithm cannot be applied on negative dividends or divisors.\n\n");
        exit(-1);
    }
...
void BinDiv_Restoring( char *Q, char *M){
    ...
    while(count>0){
        ...
        if( A[0] == '1') {
            ...
            bin_add(A,M,temp);
            strcpy(A, temp);
            printf("\n%25s%28s%28s%10d", "Q_0 <- 0, A <- A+M", A, Q, --count);
        }
        else{
            Q[n-1]='1';
    ...
    free(A); free(twos_comp_M); free(temp);
